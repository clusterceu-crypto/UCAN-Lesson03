...
## Patch 1.2.2
- кнопка Gemini відображається окремим рядком під ChatGPT у кожній картці ШІ;
- промпт «Перевірити результат» формується з фактично заповнених полів «Портфеля мера» і результатів самоперевірки;
- порожні поля не додаються до промпту;
- для кожного з шести пунктів самоперевірки додано конкретний приклад про енергоефективність шкіл, який з'являється лише для відповіді «Потрібно уточнити»;
- на завершальній сторінці «Повернутися на початок» розміщено ліворуч, а «Перейти до заняття 04» — праворуч;
- додано cache-busting параметр v1.2.2 для JavaScript-файлів.
